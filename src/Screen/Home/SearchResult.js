...
import { getBusDetailsData, getBusReviewData, setBusDepartureId, setBusProviderName, setBusProviderNameReturn, setBusReturnId } from './Redux/HomeAction';
...
export default function SearchResult(props) {
  ...
  const dataReturn = useSelector(state => { return state.HomeReducer.searchResultReturn })
  ...
  const isOneWay = useSelector(state => { return state.HomeReducer.isOneWay })
    ...
    <>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.jalurTanggal}>
          <Text style={styles.jalur}>{isReturn ? arrivalCity : departureCity} ➜ {isReturn ? departureCity : arrivalCity}</Text>
          <Text style={styles.tanggal}>{isReturn ? returnDate : departureDate}</Text>
          {isReturn ?
